...
def CombinedQloss(params, condition, cyc):
    ...
    # Q3 component
    Chm0_params  = params[12:16]
    Dhm0_params  = params[16:20]
    F0 = params[20]

    Q3_inner = ( 
                 Q1DchCrate(Chm0_params, ChCrate) +
                 Q1DchCrate(Dhm0_params, DchCrate) )
    Q3_out = F0 * ( np.exp(Q3_inner * cyc) - 1 )
...
Chm0_initial_guess = [1, 2, 0.5, 30]
Dhm0_initial_guess = [1, 2, 0.2, 30]
F0 = [0.0000000001]
Q3_initial_guess = ( Chm0_initial_guess + Dhm0_initial_guess + F0)
...
def weighted_objective(params, weights, loss_data, condition_data):
    ...
    for condition in condition_data:
        Temp, ChCrate, DchCrate = condition
        fm0_charge = Q1ChCrate(params[12:16], ChCrate)
        fm0_dch    = Q1DchCrate(params[16:20], DchCrate)
        if fm0_charge <= 0:
